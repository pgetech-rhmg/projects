...
public sealed class DiagController(IAdoService adoService, IGitHubService gitHubService, EpicDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> TestAdoRaw(string appName, CancellationToken ct)
    {
        try
        {
            ...
            var url = $"https://dev.azure.com/pgetech/EPIC-Pipeline/_apis/build/builds?definitions=194&tagFilters={Uri.EscapeDataString($"epicRepo.{appName}")}&$top=5&queryOrder=finishTimeDescending&api-version=7.1";
