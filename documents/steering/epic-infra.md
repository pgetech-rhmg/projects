...
- **Footguns**:
  - **Every direct call to this module REQUIRES a `providers = { aws.us_east_1 = aws.us_east_1 }` block at the call site.** The module's `versions.tf` declares `aws.us_east_1` as a required configuration alias because CloudFront ACM certs must live in us-east-1. Calling without this block fails `terraform init` with `Error: Missing required provider configuration`, and earlier Terraform versions emit `Warning: Reference to undefined provider` and silently fall back to the default `aws` provider — which deploys the cert in the wrong region. This means `provider "aws" { alias = "us_east_1", region = "us-east-1" }` must be present in `terraform.tf` whenever you call `aws-cloudfront` directly. (If you go through `aws-static-web`, that module passes the alias through for you — but you still need the `aws.us_east_1` provider declared in your root `terraform.tf`.)
...
- **The `aws-cloudfront` module call also needs `providers = { aws.us_east_1 = aws.us_east_1 }`** at the call site. Even if you don't pass a `custom_acm_certificate_arn`, the module declares `aws.us_east_1` as a required configuration alias, and `terraform init` errors without the providers block. See `aws-cloudfront` footguns.
