...
Every app team gets the same pipeline. They don't build one, maintain one, or debug one. They provide a small config file (`.pipeline/epic.json`) that tells EPIC what to build, test, scan, deploy and where. That's it.

EPIC handles the rest: authentication, testing gates, artifact management, and deployment to whatever cloud you choose.
