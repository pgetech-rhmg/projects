...
# -----------------------------
locals {
  hostingFileServer = base64encode(templatefile("${path.module}/templates/3_hosting-server-fileserver.json.tpl", {
    run_as_user            = var.run_as_user
    hosting_fs_directories = jsonencode(var.hosting_fs_directories)
    hosting_fs_shares      = jsonencode(var.hosting_fs_shares)
  }))

  hostingServer = base64encode(templatefile("${path.module}/templates/4_hosting-server.json.tpl", {
  ...
  hosting_ssm_document_content = templatefile("${path.module}/ssm/2_hostingConfig.json.tpl", {
...
# -----------------------------
locals {
  debug_hosting_ssm_document_content = templatefile("${path.module}/ssm/2_hostingConfig.json.tpl", {
    hostingFileServer = local.hostingFileServer
    hostingServer     = local.hostingServer
