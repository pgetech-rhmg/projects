...

    {
      ...
      "inputs": {
        "runCommand": [
          ...
          "",
          "mkdir -p \"${go_certs_dir}\"",
          "aws s3 cp \"s3://${s3_bucket}/${go_root_cert_name}\" \"${go_certs_dir}/${go_root_cert_name}\" --region ${s3_region} || (echo 'Failed to download root cert' >&2; exit 1)",
          "aws s3 cp \"s3://${s3_bucket}/${go_pfx_cert_name}\"  \"${go_certs_dir}/${go_pfx_cert_name}\" --region ${s3_region} || (echo 'Failed to download pfx cert' >&2; exit 1)",
          "",
          "mkdir -p \"${go_license_dir}/${esri_version}\"",
          "aws s3 cp \"s3://${s3_bucket}/${go_license_name}\" \"${go_authorization_file}\" --region ${s3_region} || (echo 'Failed to download license' >&2; exit 1)",
          "",
...

    {
      ...
      "inputs": {
        "runCommand": [
          ...
          "",
          "echo '${goFileServer}' | base64 --decode > /opt/cinc/attributes/go-server-fileserver.json",
          "cinc-client -z -c /etc/cinc/client.rb -j /opt/cinc/attributes/go-server-fileserver.json 2>&1 | tee -a \"$LOG_DIR/go-server-fileserver.log\"",
          "RC=$${PIPESTATUS[0]}",
...

    {
      ...
      "inputs": {
        "runCommand": [
          ...
          "",
          "echo '${goServer}' | base64 --decode > /opt/cinc/attributes/go-server.json",
          "cinc-client -z -c /etc/cinc/client.rb -j /opt/cinc/attributes/go-server.json 2>&1 | tee -a \"$LOG_DIR/go-server.log\"",
          "RC=$${PIPESTATUS[0]}",
