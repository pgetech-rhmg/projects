...
locals {
  ...
  dataStore = base64encode(templatefile("${path.module}/templates/5_arcgis-datastore-primary.json.tpl", {
  ...
  ds_ssm_document_content = templatefile("${path.module}/ssm/3_dsConfig.json.tpl", {
...
# -----------------------------
locals {
  debug_ds_ssm_document_content = templatefile("${path.module}/ssm/3_dsConfig.json.tpl", {
    dataStore = local.dataStore
    s3_bucket = var.s3_bucket
