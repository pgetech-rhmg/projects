...
# -----------------------------
locals {
  arcgisPortalFileServer = base64encode(templatefile("${path.module}/templates/1_arcgis-portal-fileserver.json.tpl", {
  ...
  arcgisPortalPrimary = base64encode(templatefile("${path.module}/templates/2_arcgis-portal-primary.json.tpl", {
    esri_version                           = var.esri_version
    run_as_user                            = var.run_as_user
  ...
  portal_ssm_document_content = templatefile("${path.module}/ssm/1_portalConfig.json.tpl", {
...
# -----------------------------
locals {
  debug_portal_ssm_document_content = templatefile("${path.module}/ssm/1_portalConfig.json.tpl", {
    arcgisPortalFileServer = local.arcgisPortalFileServer
    arcgisPortalPrimary    = local.arcgisPortalPrimary
