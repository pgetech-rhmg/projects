##############################################################################################
#                           Hosted Server Machines Creation Module                           #
##############################################################################################
...
module "hosted-server-ec2" {
  ...
  #count   = var.HostedServerMachineCount

  name                   = var.hostedservermachinename
  ami                    = var.hosted_server_ami # "Hardcode the AMI ID here to avoid terraform destroy of old machine if ami id changes in SSM"
  instance_type          = var.hostedserverinstancetype
  ...
  root_block_device = [
    {
      ...
      volume_size = var.hostedserverprimaryvolume
  ...
  tags = merge(module.tags.tags, local.optional_tags, { Name = var.hostedservermachinename, SoftwareComponent = "arcgisserver", ServerRole = "Primary" })
...
module "hosted-efs" {
  ...
  tags            = merge(module.tags.tags, local.optional_tags, { Name = var.hostedefsname} )
}

#################################################
# Server Siteadmin Secret key
#################################################

module "serversecretsmanager" {
  source  = "app.terraform.io/pgetech/secretsmanager/aws"
  version = "0.1.3"

  secretsmanager_name        = var.server_secretsmanager_name
  secretsmanager_description = var.server_secretsmanager_description
  ...
  secret_string              = random_password.server_admin_password.result
