{
  "Comment": "AMI Factory Build Pipeline - TEST MODE (uses existing AMIs)",
  "StartAt": "MockBuildAMIs",
  "States": {
    "MockBuildAMIs": {
      "Type": "Map",
      ...
      "ItemProcessor": {
        ...
        "States": {
          "SaveComponentName": {
            ...
            "Next": "LookupExistingAMI"
          },
          "LookupExistingAMI": {
            "Type": "Task",
            "Resource": "arn:aws:states:::aws-sdk:ec2:describeImages",
            "Comment": "Get the most recent AMI for this component",
            "Parameters": {
              "Owners": ["self"],
              "Filters": [
                {
                  "Name": "name",
                  "Values.$": "States.Array(States.Format('ami-factory-{}-*', $.component))"
                }
              ]
            },
            "ResultPath": "$.lookup",
          ...
          "FormatOutput": {
            ...
            "Parameters": {
              "component.$": "$.component",
              "ami_id.$": "$.lookup.Images[0].ImageId",
              "ami_name.$": "$.lookup.Images[0].Name",
              "region": "${aws_region}"
            },
            "End": true
          }
