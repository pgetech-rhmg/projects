...
variable "gateway_load_balancer_arns" {
  description = "A list of gateway_load_balancer_arns"
  type        = list(string)
  validation {
    condition = anytrue([
      for i in var.gateway_load_balancer_arns : can(regex("^arn:aws:elasticloadbalancing:\\w+(?:-\\w+)+:[[:digit:]]{12}:loadbalancer/gwy/([a-zA-Z0-9])+(.*)$", i))
    ])
    error_message = "Gateway_load_balancer_arns is required and the allowed format of 'gateway_load_balancer_arns' is arn:aws:elasticloadbalancing:<region>:<account-id>:loadbalancer/gwy/<gateway-loadbalancer-id>."
  }
}
