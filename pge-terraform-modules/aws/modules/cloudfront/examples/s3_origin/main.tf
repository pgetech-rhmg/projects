...
*/
#
# Filename    : aws/modules/cloudfront/examples/cloudfront_s3_origin/main.tf
# Date        : 25 april 2022
# Author      : TCS
# Description : Terraform module example for cloudfront S3 orgin.
#
...
module "cloudfront" {
  ...
  origin = [{
    ...
    s3_origin_config = [{
      origin_access_identity = module.cloudfront_oai.origin_access_identity_cloudfront_access_identity_path
    }]
  }]
...
module "s3_origin_bucket" {
  ...
  policy      = templatefile("${path.module}/${var.s3_policy_origin}", { oai_origin = module.cloudfront_oai.origin_access_identity_iam_arn, account_num = data.aws_caller_identity.current.account_id, aws_role = var.aws_role, bucket = var.origin_bucket_name })
  tags        = merge(module.tags.tags, local.optional_tags)
