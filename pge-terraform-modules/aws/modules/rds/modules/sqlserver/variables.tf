...
variable "engine" {
  ...
  validation {
    ...
    error_message = "Valid values for engine are sqlserver-ee, sqlserver-se, sqlserver-ex, sqlserver-web"
  }
}
...
variable "db_name" {
  description = "The DB name to create. If omitted, no database is created initially.  For SQL Server, this variable may need to be set to null."
  type        = string
  default     = null
}
...
variable "create_iam_role_association" {
  description = "Create IAM role association for S3 integration"
  type        = bool
  default     = false
}
