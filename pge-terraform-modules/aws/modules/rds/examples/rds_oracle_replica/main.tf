 ...
 *    8. RDS VPC Endpoint exists for the account.  If not, you can deploy one in the account as shown with the rds_vpc_endpoint example module (rds/examples/rds_vpc_endpoint)
...
*/
#
#  Filename    : modules/rds/examples/rds_oracle_replica/main.tf
#  Date        : 10/5/2023
#  Author      : PGE
#  Description : RDS Oracle Module read replica creation main file.
#
terraform {
...
module "oracle_rds_replica" {
  source     = "../../modules/rds_instance_replica"
  identifier = "oracle-rds-replica-${var.identifier}"
  # Source database. For cross-region use db_instance_arn
  replicate_source_db             = one(module.oracle_rds[*].db_instance_id)
  engine                          = var.engine
  engine_version                  = var.engine_version
  instance_class                  = var.instance_class
  allocated_storage               = var.allocated_storage
  ...
  password                        = var.password
  multi_az                        = false
  vpc_security_group_ids          = [one(module.oracle_rds[*].security_group_id)]
  monitoring_role_arn             = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/rds-enhanced-monitoring-role"
  maintenance_window              = var.maintenance_window
  backup_window                   = var.backup_window
  enabled_cloudwatch_logs_exports = var.enabled_cloudwatch_logs_exports
