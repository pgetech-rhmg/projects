...
#
#  Filename    : modules/rds/examples/rds_sqlserver_atuomated_backups_replication/main.tf
#  Date        : 5/26/2022
#  Author      : PGE
#  Description : Example of RDS SQL Server with automated backup replication module
#
terraform {
...
module "rds_automated_backups_replication" {
  ...
  source_db_instance_arn = module.rds_sqlserver.db_instance_arn
  kms_key_arn            = null # replace aws_kms_replica_key.replica.arn when var.storage_encrypted is set to true
  providers = {
    aws = aws.replica
  }
}
