module "sonarqube" {
  ...
  version                = "0.0.10"
  codebuild_project_name = "sonarqube_${local.repo_name}-${local.suffix}"
  ...
  codebuild_resource_policy = jsonencode({
    ...
    "Statement" : [{
      ...
      "Resource" : "arn:aws:codebuild:${data.aws_region.current.name}:${data.aws_caller_identity.current.id}:project/sonarqube_${local.repo_name}-${local.suffix}"
