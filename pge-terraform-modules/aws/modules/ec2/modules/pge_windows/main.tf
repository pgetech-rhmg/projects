...
#  Description : Creation of ec2 with windows defined role. EBS will be encrypted with CMEK. RootBlock Device can be encrypted with AWS managed or CMEK. 
#  
#
...
locals {
  ...
  pge_windows_ami_tags     = { pge_ami_cn = "XXXX", pge_ami_sn = "YYYY", pge_ami_os = "ZZZZ" }
  default_windows_iam_role = "windows-ec2-golden-role"
  monitoring               = true
  name                     = var.name
  instance_profile_role    = var.instance_profile_role != null ? var.instance_profile_role : local.default_windows_iam_role
  module_tags              = merge(var.tags, { pge_team = local.namespace, tfc_wsname = module.ws.name, tfc_wsid = module.ws.id }, local.pge_windows_ami_tags)
}
...
resource "aws_instance" "this" {
  ...
  dynamic "root_block_device" {
    ...
    content {
      ...
      kms_key_id            = lookup(root_block_device.value, "kms_key_id", null) # KMS_KEY_ID is mandatory if the DataClassification is not "Internal".
  ...
  dynamic "ebs_block_device" {
    ...
    content {
      ...
      kms_key_id            = ebs_block_device.value.kms_key_id #KMS_KEY_ID is mandatory if the DataClassification is not "Internal".
  ...
  lifecycle {
    ignore_changes = [tags["pge_ami_sn"], tags["pge_ami_os"], tags["pge_ami_cn"]]
  }
