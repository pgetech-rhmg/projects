...
output "role_arn" {
  ...
  value       = var.instance_profile_role == null ? module.aws_iam_role[0].arn : data.aws_iam_role.pge_linux_role.arn
}

output "role_name" {
  description = "The name of the instance IAM profile role created"
  value       = var.instance_profile_role == null ? module.aws_iam_role[0].name : data.aws_iam_role.pge_linux_role.name
}
