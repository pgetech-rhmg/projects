...
variable "automatic_failover_enabled" {
  description = "Specifies whether a read-only replica will be automatically promoted to read/write primary if the existing primary fails. If enabled, number_cache_clusters must be greater than 1. Must be enabled for 'cluster mode enabled' replication groups. Defaults to false"
  type        = string
  default     = null
}
...
variable "replica_notification_topic_arn" {
  ...
  validation {
    condition = anytrue([
      var.replica_notification_topic_arn == null,
      can(regex("^arn:aws:sns:\\w+(?:-\\w+)+:[[:digit:]]{12}:([a-zA-Z0-9])+(.*)$", var.replica_notification_topic_arn))
    ])
    error_message = "Amazon SNS topic ARN is required and allowed format of the Amazon SNS topic ARN is arn:aws:sns:<region>:<account-id>:<sns-topic-name>."
  }
}
...
variable "replica_snapshot_name" {
  description = " Name of a snapshot from which to restore data into the new node group. Changing the snapshot_name forces a new resource."
  type        = string
  default     = null
}
