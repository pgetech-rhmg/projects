...
variable "sqs_name" {
  type        = string
  description = "The SQS queue name for fifo queue"

  validation {
    condition = alltrue([
      can(regex(".fifo$", var.sqs_name)),
      can(regex("([a-zA-Z0-9-_]+)", var.sqs_name))
    ])
    error_message = "Error! Sqs name is required and allowed format of the Sqs name is ############.fifo."
  }
}
...
variable "fifo_throughput_limit" {
  ...
  validation {
    ...
    error_message = "Error! Provide valid values for fifo-throughput-limit  (perMessageGroupId, perQueue)."
  }
}
