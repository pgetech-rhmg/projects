...
| <a name="input_maintenance_window_task_type"></a> [maintenance\_window\_task\_type](#input\_maintenance\_window\_task\_type) | The type of task being registered. Valid values: AUTOMATION, LAMBDA, RUN\_COMMAND or STEP\_FUNCTIONS | `string` | `"STEP_FUNCTIONS"` | no |
...
| <a name="input_task_invocation_step_functions_parameters"></a> [task\_invocation\_step\_functions\_parameters](#input\_task\_invocation\_step\_functions\_parameters) | The parameters for a STEP\_FUNCTIONS task type. | <pre>list(object({<br>    input : string<br>    name : string<br>    }<br>    )<br>  )</pre> | `[]` | no |
