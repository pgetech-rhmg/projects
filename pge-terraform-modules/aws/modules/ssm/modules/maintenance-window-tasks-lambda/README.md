<!-- BEGIN_TF_DOCS -->
# AWS SSM module
Terraform module which creates SAF2.0 SSM Maintenance-Window Lambda Task resource in AWS
...
| <a name="input_maintenance_window_task_type"></a> [maintenance\_window\_task\_type](#input\_maintenance\_window\_task\_type) | The type of task being registered. Valid values: AUTOMATION, LAMBDA, RUN\_COMMAND or STEP\_FUNCTIONS | `string` | `"LAMBDA"` | no |
...
| <a name="input_task_invocation_lambda_parameters"></a> [task\_invocation\_lambda\_parameters](#input\_task\_invocation\_lambda\_parameters) | The parameters for a LAMBDA task type. | <pre>list(object({<br>    client_context : string<br>    payload : string<br>    }<br>    )<br>  )</pre> | `[]` | no |
