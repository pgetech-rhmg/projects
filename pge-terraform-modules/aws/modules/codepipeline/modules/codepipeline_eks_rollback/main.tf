...
*/
#
#  Filename    :  aws/modules/codepipeline/modules/codepipeline_ecs_rollback/main.tf
...
resource "aws_codepipeline" "codepipeline" {
  ...
  # Stage 3 Helmchart revision rollback

  stage {
    name = "HelmchartRollback"
    action {
      name             = "helmchart-rollback"
      category         = "Build"
      owner            = "AWS"
      provider         = "CodeBuild"
      ...
      run_order        = 1
      configuration = {
        ProjectName = module.helmchart_rollback.codebuild_project_id
      }
    }
  }
