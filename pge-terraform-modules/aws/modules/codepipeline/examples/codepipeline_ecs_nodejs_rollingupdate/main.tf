...
* # Prerequisites : In the variables 'ssm_parameter_vpc_id', 'ssm_parameter_subnet_id1/2/3', 'ssm_parameter_artifactory_host', 'ssm_parameter_artifactory_nodejs_repo', 'ssm_parameter_sonar_host', 'ssm_parameter_twistlock_console', 'ssm_parameter_environment', 'ssm_parameter_artifactory_docker_repo', 'secretsmanager_github_token_secret_name', 'secretsmanager_artifactory_token','secretsmanager_artifactory_user', 'secretsmanager_sonar_token', 'secretsmanager_twistlock_user_id', 'secretsmanager_twistlock_token'.  Provide the suitable values respectively in tfvars.
* # Code verified using terraform validate and terraform fmt -check.
*/
#
#  Filename    : aws/modules/codepipeline/examples/codepipeline_ecs_nodejs_rollingupdaten/main.tf
...
module "codepipeline" {
  source             = "../../modules/codepipeline_ecs_nodejs"
  codepipeline_name  = var.codepipeline_name
  ...
  stages = [

    {
      ...
      action = [
        {
          name            = var.deployment_stage_name
          category        = "Deploy"
          owner           = "AWS"
          version         = "1"
  ...
  environment_variables_codebuild_stage = [
    ...
    },
    {
      ...
      value = data.aws_ssm_parameter.artifactory_nodejs_repo.value
      type  = "PLAINTEXT"
    },
    {
  ...
  artifactory_nodejs_repo = data.aws_ssm_parameter.artifactory_nodejs_repo.value
  sonar_host              = data.aws_ssm_parameter.sonar_host.value
  artifactory_docker_repo = data.aws_ssm_parameter.artifactory_docker_repo.value
