...
variable "aws_org_id" {
  type        = string
  description = "The AWS organization id that this pipeline instance is scoped to manage."
}
...
variable "secretsmanager_ccoe_ssm_patch_pat" {
  description = "secret manager path of the CCOE github OAUTH or PAT"
  type        = string
}
...
variable "secretsmanager_terraform_token" {
  description = "secret manager key for the terraform token value."
  type        = string
}
...
variable "s3_kms_key_arn" {
  description = "KMS key arn for data encryption - a pre-requisite"
  type        = string
}
...
variable "codebuild_additional_policy" {
  description = "Additional privies for the codebuild role."
  type        = string
}
