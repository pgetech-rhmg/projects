<!-- BEGIN_TF_DOCS -->
# AWS codepipeline Java User module example with downloading artifact from artifactory.
Enabling download\_artifact variable to true will skip following stages from codepipeline\_java module
1. build,
2. **sonarscan analysis**,
3. publish
and a stage will get created to download previously uploaded artifact from artifactory (see Prerequisites below).
Prerequisites :
1. set version.toml in your source code repo with artifact details, see here for example - https://github.com/pgetech/terraform-cicd-pipeline-ref/blob/main/reference-pipelines/java/ec2/springboot-war-example/version.toml
2. Run Codepipeline\_java example in dev environments which runs build, scan, publish stages (uploads artifact to artifactory).
...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_codebuild_codetest"></a> [codebuild\_codetest](#module\_codebuild\_codetest) | app.terraform.io/pgetech/codebuild/aws | 0.1.4 |
| <a name="module_codedeploy_app_ec2"></a> [codedeploy\_app\_ec2](#module\_codedeploy\_app\_ec2) | app.terraform.io/pgetech/codedeploy/aws | 0.1.1 |
| <a name="module_codepipeline"></a> [codepipeline](#module\_codepipeline) | ../../modules/codepipeline_java | n/a |
...
| <a name="module_codetest_iam_role"></a> [codetest\_iam\_role](#module\_codetest\_iam\_role) | app.terraform.io/pgetech/iam/aws | 0.1.0 |
| <a name="module_ec2"></a> [ec2](#module\_ec2) | app.terraform.io/pgetech/ec2/aws | 0.1.0 |
...
| [aws_iam_policy.ec2_kms_policy](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_policy) | resource |
...
| <a name="input_cidr_egress_rules_codebuild"></a> [cidr\_egress\_rules\_codebuild](#input\_cidr\_egress\_rules\_codebuild) | n/a | <pre>list(object({<br>    from             = number<br>    to               = number<br>    protocol         = string<br>    cidr_blocks      = list(string)<br>    ipv6_cidr_blocks = list(string)<br>    prefix_list_ids  = list(string)<br>    description      = string<br>  }))</pre> | n/a | yes |
...
| <a name="input_codetest_role_name"></a> [codetest\_role\_name](#input\_codetest\_role\_name) | Name of the iam role for codepublish project | `string` | n/a | yes |
| <a name="input_compute_type_codedownload"></a> [compute\_type\_codedownload](#input\_compute\_type\_codedownload) | Information about the compute resources the build project will use in codedownload project | `string` | n/a | yes |
...
| <a name="input_concurrent_build_limit_codedownload"></a> [concurrent\_build\_limit\_codedownload](#input\_concurrent\_build\_limit\_codedownload) | Maximum number of concurrent builds for the codedownload project | `number` | n/a | yes |
...
| <a name="input_custom_codebuild_policy_file"></a> [custom\_codebuild\_policy\_file](#input\_custom\_codebuild\_policy\_file) | custom policy file | `string` | `null` | no |
...
| <a name="input_dependency_file_location"></a> [dependency\_file\_location](#input\_dependency\_file\_location) | Enter the location of dependency file - variable in buildspec yml | `string` | n/a | yes |
...
| <a name="input_deployment_tag_value"></a> [deployment\_tag\_value](#input\_deployment\_tag\_value) | CodeDeploy Deployment Tag Value | `string` | n/a | yes |
...
| <a name="input_ec2_instance_type"></a> [ec2\_instance\_type](#input\_ec2\_instance\_type) | Instance type of the ec2 instance | `string` | n/a | yes |
| <a name="input_ec2_name"></a> [ec2\_name](#input\_ec2\_name) | The name for ec2 instance | `string` | n/a | yes |
| <a name="input_environment_image_codedownload"></a> [environment\_image\_codedownload](#input\_environment\_image\_codedownload) | Docker image to use for this codedownload project. Valid values include Docker images provided by CodeBuild (e.g aws/codebuild/standard:2.0), Docker Hub images (e.g., hashicorp/terraform:latest), and full Docker repository URIs such as those for ECR (e.g., 137112412989.dkr.ecr.us-west-2.amazonaws.com/amazonlinux:latest). | `string` | n/a | yes |
...
| <a name="input_environment_type_codedownload"></a> [environment\_type\_codedownload](#input\_environment\_type\_codedownload) | Type of build environment to use for codedownload project related builds. Valid values: LINUX\_CONTAINER, LINUX\_GPU\_CONTAINER, WINDOWS\_CONTAINER (deprecated), WINDOWS\_SERVER\_2019\_CONTAINER, ARM\_CONTAINER. | `string` | n/a | yes |
...
| <a name="input_java_runtime"></a> [java\_runtime](#input\_java\_runtime) | enter the java runtime version - variable in buildspec yml | `string` | n/a | yes |
...
| <a name="input_sg_name_codebuild"></a> [sg\_name\_codebuild](#input\_sg\_name\_codebuild) | name of the security group | `string` | n/a | yes |
...
| <a name="input_source_location_codedownload"></a> [source\_location\_codedownload](#input\_source\_location\_codedownload) | Location of the source code from git or s3 for codedownload. | `string` | n/a | yes |
...
| <a name="input_ssm_parameter_golden_ami_name"></a> [ssm\_parameter\_golden\_ami\_name](#input\_ssm\_parameter\_golden\_ami\_name) | The name given in parameter store for the golden ami | `string` | n/a | yes |
