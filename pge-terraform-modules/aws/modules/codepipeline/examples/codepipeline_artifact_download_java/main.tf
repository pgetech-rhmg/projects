 ...
 * Enabling download_artifact variable to true will skip following stages from codepipeline_java module
 * 1. build,
 ...
 * 2. Run Codepipeline_java example in dev environments which runs build, scan, publish stages (uploads artifact to artifactory).
*/
#
#  Filename    : aws/modules/codepipeline/examples/codepipeline_artifact_download/main.tf
#  Date        : 27 August 2024
#  Author      : ccoe-cicd
#  Description : The terraform example creates a codepipeline downloading artifact from artifactory
...
module "codepipeline" {
  source            = "../../modules/codepipeline_java"
  ...
  stages = [
    {
      ...
      action = [{
        ...
        owner            = "AWS"
        provider         = "CodeBuild"
        input_artifacts  = ["Download_Artifact"]
  ...
  custom_codedownload_policy_file     = "${path.module}/custom_codedownload_policy_file.json"
  ...
  dependency_file_location = var.dependency_file_location
...
module "codebuild_codetest" {
  ...
  codebuild_sc_token           = var.secretsmanager_github_token_secret_name
  codebuild_resource_policy    = templatefile("${path.module}/codetest_inline_policy.json", { account_num = data.aws_caller_identity.current.id, partition = data.aws_partition.current.partition, aws_region = data.aws_region.current.name, codebuild_project_name = "codetest_project_${var.codepipeline_name}" })
...
module "s3" {
  ...
  kms_key_arn   = null #if you want to encrypt the data, replace with module.kms_key.key_arn, after key creation
  tags          = merge(module.tags.tags, local.optional_tags)
}
  ...
  root_block_device = [
    {
      ...
      kms_key_id  = null #if you want to encrypt the data, replace with module.kms_key.key_arn, after key creation
    },
  ]
