#
#  Filename    : aws/modules/lm-lambda-pipeline/codebuild.tf
#  Date        : 15 April 2025
...
module "codebuild_build" {
  ...
  version                       = "0.1.11"
  codebuild_project_name        = "${var.lambda_name}_codebuild_build"
  codebuild_project_role        = module.codebuild_build_iam_role.arn
  concurrent_build_limit        = 1
  codebuild_project_description = "Build step for ${var.lambda_name}"
  ...
  codebuild_resource_policy = templatefile("${path.module}/iam_policies/codebuild_build_policy.json", { account_num = data.aws_caller_identity.current.account_id, partition = data.aws_partition.current.partition, aws_region = data.aws_region.current.name, codebuild_project_name = "${var.lambda_name}_codebuild_build" })
...
module "codebuild_sonar" {
  ...
  version                = "0.0.10"
  codebuild_project_name = "${var.lambda_name}_codebuild_sonar"
  artifact_type          = "CODEPIPELINE"
  source_type            = "CODEPIPELINE"
  environment_image      = var.codebuild_image
  environment_type       = "LINUX_CONTAINER"
  ...
  codebuild_resource_policy = jsonencode({
    ...
    "Statement" : [{
      ...
      "Resource" : "arn:aws:codebuild:${data.aws_region.current.name}:${data.aws_caller_identity.current.id}:project/${var.lambda_name}_codebuild_sonar"
    }]
  })
...
module "codebuild_deploy" {
  source                 = "app.terraform.io/pgetech/codebuild/aws//modules/codebuild_project"
  version                = "0.0.10"
  codebuild_project_name = "${var.lambda_name}_codebuild_deploy"
  artifact_type          = "CODEPIPELINE"
  source_type            = "CODEPIPELINE"
  environment_image      = var.codebuild_image
  environment_type       = "LINUX_CONTAINER"
  ...
  compute_type           = "BUILD_GENERAL1_LARGE"
  tags                   = module.tags.tags
  source_buildspec = templatefile("${path.module}/buildspecs/buildspec_deploy.yml", {
    lambda_name = var.lambda_name
  })
  vpc_id             = data.aws_ssm_parameter.vpc_id.value
  subnet_ids         = [for s in values(data.aws_ssm_parameter.subnets) : s.value]
  ...
  codebuild_resource_policy = jsonencode({
    ...
    "Statement" : [{
      ...
      "Resource" : "arn:aws:codebuild:${data.aws_region.current.name}:${data.aws_caller_identity.current.id}:project/${var.lambda_name}_codebuild_deploy"
    }]
  })
