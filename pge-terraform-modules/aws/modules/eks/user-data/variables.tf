...
variable "ami_type" {
  description = "AL2023_x86_64_STANDARD Type of Amazon Machine Image (AMI) associated with the EKS Node Group. See the [AWS documentation](https://docs.aws.amazon.com/eks/latest/APIReference/API_Nodegroup.html#AmazonEKS-Type-Nodegroup-amiType) for valid values"
  type        = string
  default     = ""
  nullable    = false
}
...
variable "additional_cluster_dns_ips" {
  ...
  description = "Additional DNS IPs to add to the cluster DNS IPs list"
  type        = list(string)
  default     = []
  nullable    = false
}
