module "sonarqube" {
  ...
  version                = "0.0.10"
  codebuild_project_name = "${lower(local.prefix)}-sonarqube-graph-${lower(local.suffix)}"
  ...
  tags                   = var.tags
  source_buildspec       = file("${path.module}/pipeline-buildspecs/sonarqube.yaml")
  ...
  codebuild_resource_policy = jsonencode({
    ...
    "Statement" : [{
      ...
      "Resource" : "arn:aws:codebuild:${data.aws_region.current.name}:${data.aws_caller_identity.current.id}:project/${lower(local.prefix)}-sonarqube-graph-${lower(local.suffix)}"
  ...
  environment_variables = [
    ...
    {
      name  = "BRANCH"
      value = nonsensitive(data.github_branch.graph_current_branch.branch)
      type  = "PLAINTEXT"
    },
    {
      name  = "SONAR_HOST"
