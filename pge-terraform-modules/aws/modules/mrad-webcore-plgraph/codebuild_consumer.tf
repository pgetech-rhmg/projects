resource "aws_codebuild_project" "consumer" {
  name          = "${lower(local.prefix)}-build-consumer-${lower(local.suffix)}"
  description   = "Build and push the Consumer docker image"
  build_timeout = "50"
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = file("${path.module}/pipeline-buildspecs/consumer.yaml")
  }
