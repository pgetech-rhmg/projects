...
#
#  Filename    : aws/module/alb/modules/internal_alb/main.tf
#  Date        : 22 March 2022
#  Author      : TCS
#  Description : This terraform module creates internal alb.
...
locals {
  ...
  lb_listener_https_fixed_response = flatten([for index, value in var.lb_listener_https : [
  for ki, va in value : merge({ fixed_response_index = index }, va) if ki == "fixed_response"]])
...
resource "aws_lb_listener" "lb_listener_http" {
  ...
  default_action {
    ...
    dynamic "redirect" {
      for_each = length(local.lb_listener_http_redirect) > 0 ? [local.lb_listener_http_redirect[0]] : []
...
resource "aws_lb_listener" "lb_listener_https" {
  ...
  default_action {
    ...
    dynamic "redirect" {
      for_each = length(local.lb_listener_https_redirect) > 0 ? [local.lb_listener_https_redirect[0]] : []
    ...
    dynamic "fixed_response" {
      for_each = [for index, fr in local.lb_listener_https_fixed_response : fr if fr.fixed_response_index >= 0]
...
resource "aws_lb_listener_rule" "lb_listener_rule_https" {
  ...
  listener_arn = aws_lb_listener.lb_listener_https[each.value.lb_listener_https_port].arn #index rename
  priority     = lookup(each.value, "priority", null)
