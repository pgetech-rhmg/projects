...
*/
#
#  Filename    : sonar-token-renewal/main.tf
#  Date        : 10 Jan 2024
#  Author      : Mounika Kota
#  Description : AWS Lambda function with Environment Variables for renewing sonarqube token before expiration
...
#########################################
#lambda function which calls sonarqube api to generate new token
module "lambda_function" {
  ...
  environment_variables = {
    variables = {
      sonar_token_name_new          = var.sonar_token_name_new
      sonar_secretsmanager_name     = var.secretsmanager_name
      sonar_host                    = var.sonar_host
      secrets_manager_token_keyname = var.secrets_manager_token_keyname
...
module "iam_policy" {
  ...
  name        = "kms-policy-sonar-token-renewal"
  path        = "/"
  description = "Kms encrypt decrypt policy for sonar token renewal lambda"
