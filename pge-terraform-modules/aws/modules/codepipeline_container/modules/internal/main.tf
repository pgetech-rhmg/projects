/*
 * # AWS codepipeline for Container based application module
 * Terraform module which creates SAF2.0 codepipeline in AWS
*/
#
#  Filename    :  aws/modules/codepipeline_container/modules/internal/main.tf
#  Date        : Dec 20 2024
#  Author      : PGE
#
...
resource "aws_codepipeline" "codepipeline" {
  depends_on = [
    module.codepipeline_internal_codestar_notifications.lambda_arn
  ]
  ...
  stage {
    ...
    dynamic "action" {
      for_each = var.source_type == "GitHub" ? [true] : []
      content {
        name             = "Secrets-Source"
        category         = "Source"
        owner            = "ThirdParty"
  ...
  stage {
    ...
    action {
      ...
      output_artifacts = var.codedeploy_provider == "CodeDeployToECS" ? ["ImageArtifact", "DefinitionArtifact"] : ["wiz-scan"]
      region           = var.region
      configuration = {
        ProjectName = module.codebuild_wiz.codebuild_project_id
      }
    }
  }
