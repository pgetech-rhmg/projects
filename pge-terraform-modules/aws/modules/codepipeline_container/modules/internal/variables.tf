...
variable "artifactory_python_repo" {
  description = "Enter the name value of JFrog Artifactory"
  type        = string
  default     = null
}
...
variable "artifactory_helm_local_repo" {
  description = "Enter the name value of JFrog Artifactory helm chart virtual group name stored in SSM parameter"
  type        = string
  default     = ""
}
...
variable "build_command" {
  description = "Enter the build or install command to build nodejs webapp application"
  type        = string
  default     = ""
}
...
variable "cache_type_wiz" {
  description = "Type of storage that will be used for the AWS CodeBuild project cache for wiz"
  type        = string
  default     = "NO_CACHE"
  validation {
    condition     = var.cache_type_wiz == "NO_CACHE" || var.cache_type_wiz == "LOCAL" || var.cache_type_wiz == "S3"
    error_message = "Valid values are NO_CACHE, LOCAL, S3."
  }
}

variable "cache_location_wiz" {
  description = "Location where the AWS CodeBuild project stores cached resources for wiz"
  type        = string
  default     = null
}

variable "cache_modes_wiz" {
  description = "Specifies settings that AWS CodeBuild uses to store and reuse build dependencies for wiz"
  type        = string
  default     = "LOCAL_SOURCE_CACHE"
  validation {
    condition     = var.cache_modes_wiz == "LOCAL_SOURCE_CACHE" || var.cache_modes_wiz == "LOCAL_DOCKER_LAYER_CACHE" || var.cache_modes_wiz == "LOCAL_CUSTOM_CACHE"
    error_message = "Valid values are LOCAL_SOURCE_CACHE, LOCAL_DOCKER_LAYER_CACHE, LOCAL_CUSTOM_CACHE."
  }
}
...
variable "environment_image_codesecret" {
  description = "Docker image to use for CodeSecret project. Valid values include Docker images provided by CodeBuild (e.g., aws/codebuild/standard:2.0), Docker Hub images (e.g., hashicorp/terraform:latest), and full Docker repository URIs such as those for ECR (e.g., 137112412989.dkr.ecr.us-west-2.amazonaws.com/amazonlinux:latest)."
  type        = string
}

variable "environment_type_codesecret" {
  description = "Type of build environment to use for CodeSecret project related builds. Valid values: LINUX_CONTAINER, LINUX_GPU_CONTAINER, WINDOWS_CONTAINER (deprecated), WINDOWS_SERVER_2019_CONTAINER, ARM_CONTAINER."
  type        = string
}
...
# Variables for CodeStar configuration
...
# Variables for Eks variables
variable "is_eks_fargate" {
  description = "To embed wiz defender if the cluster is EKS Fargate"
  type        = bool
}

# Variables for runtimes
...
variable "nodejs_runtime_codescan" {
  description = "Enter the Node.js version value for CodeScan. Minimum of Node 18 version is required to run SonarScan. Latest LTS is 20 which is recommended."
  type        = string
  default     = "20"
  validation {
    condition     = can(regex("(1[8-9]|[2-9][0-9])", var.nodejs_runtime_codescan))
    error_message = "Minimum of Node 18 version is required to run SonarScan."
  }
}
