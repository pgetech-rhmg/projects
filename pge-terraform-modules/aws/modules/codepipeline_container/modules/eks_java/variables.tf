...
variable "java_runtime" {
  description = "Enter the Java runtime version for CodeBuild stages - variable in buildspec yml"
  type        = string
}

variable "java_runtime_codescan" {
  description = "Enter the Java runtime version for CodeScan stage. Minimum of Java 17 version is required to run SonarScan - variable in buildspec yml"
  type        = string
  default     = "corretto17"
  validation {
    condition     = can(regex("^corretto(1[7-9]|[2-9][0-9])$", var.java_runtime_codescan))
    error_message = "Minimum of Java 17 version is required to run SonarScan."
  }
}

variable "runtime_version_codescan" {
  ...
  default     = "20"
  validation {
    condition     = can(regex("(1[8-9]|[2-9][0-9])", var.runtime_version_codescan))
...
variable "artifactory_maven_repo" {
  description = "Enter the name value of JFrog Artifactory"
  type        = string
  default     = null
}
...
variable "codedeploy_provider" {
  ...
  validation {
    ...
    error_message = "Valid values for ECS deployment strategies are ECS, CodeDeployToECS."
  }
}
...
# Boolean flag to determine if the cluster is EKS Fargate. If true, wiz defender will be embedded.
