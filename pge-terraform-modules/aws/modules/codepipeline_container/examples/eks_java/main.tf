...
* # Prerequisites : In the variables 'ssm_parameter_vpc_id', 'ssm_parameter_subnet_id1/2/3', 'ssm_parameter_artifactory_host', 'ssm_parameter_artifactory_nodejs_repo', 'ssm_parameter_sonar_host', 'ssm_parameter_environment', 'ssm_parameter_artifactory_docker_registry', 'secretsmanager_github_token_secret_name', 'secretsmanager_artifactory_token','secretsmanager_artifactory_user', 'secretsmanager_sonar_token', 'secretsmanager_wiz_client_id', 'secretsmanager_wiz_client_secret', 'argocd_repo_name', 'argocd_repo_path', 'secretsmanager_codebuild_github_token_secret_name'.  Provide the suitable values respectively in tfvars.
* # Code verified using terraform validate and terraform fmt -check.
*/
#
#  Filename    : aws/modules/codepipeline_container/examples/eks_java/main.tf
#  Date        : 04 Nov 2022
#  Author      : Tekyantra
#  Description : The terraform module creates a container eks_Java
  ...
  stages = [
    {
      ...
      action = [{
        ...
        run_order        = 1
        configuration = {
          ProjectName = module.argocd_deployment.codebuild_project_id
        }
      }]
    }
  ]
...
module "argocd_deployment" {
  ...
  environment_image            = var.environment_image_argocd_deploy
  environment_type             = var.environment_type_argocd_deploy
  artifact_location            = module.s3.id
  artifact_bucket_owner_access = var.artifact_bucket_owner_access
  artifact_path                = var.artifact_path
  source_location              = var.source_location_argocd_deploy
  concurrent_build_limit       = var.concurrent_build_limit_argocd_deploy
  codebuild_project_role       = module.codebuild_iam_role.arn
  compute_type                 = var.compute_type_argocd_deploy
  tags                         = merge(module.tags.tags, local.optional_tags)
  source_buildspec             = file("${path.module}/buildspec_argocd_deploy.yml")
  vpc_id                       = data.aws_ssm_parameter.vpc_id.value
  ...
  codebuild_resource_policy = templatefile("${path.module}/codebuild_inline_policy.json", {
    ...
    codebuild_project_name = "codebuild_argocd_${var.codepipeline_name}"
  })
  ...
  environment_variables = [
    {
      name  = "ARTIFACTORY_CREDS"
      value = var.secretsmanager_artifactory_secret_name
      type  = "PLAINTEXT"
    },
    {
      name  = "ARTIFACTORY_TOKEN"
    ...
    },
    {
      ...
      value = "${var.secretsmanager_codebuild_github_token_secret_name}:Token"
      type  = "SECRETS_MANAGER"
    },
    {
    ...
    },
    {
      ...
      value = var.argocd_repo_name
      type  = "PLAINTEXT"
    },
    {
      name  = "ARGOCD_REPO_PATH"
...
module "codebuild_iam_role" {
  ...
  name        = "codebuild_${var.codepipeline_name}_argocd_iam_role"
  aws_service = var.codebuild_role_service
  tags        = merge(module.tags.tags, local.optional_tags)
  inline_policy = [templatefile("${path.module}/codebuild_role_policy.json", {
    partition                            = data.aws_partition.current.partition
    aws_region                           = data.aws_region.current.name
    account_num                          = data.aws_caller_identity.current.account_id
    codebuild_project_name               = "codebuild_argocd_${var.codepipeline_name}"
    codepipeline_name                    = var.codepipeline_name
    github_token_secret_name             = var.secretsmanager_github_token_secret_name
    shared_argocd_github_pat_secret_name = var.secretsmanager_codebuild_github_token_secret_name
    jfrog_credentials_secret_name        = var.secretsmanager_artifactory_secret_name
  })]
}
