<!-- BEGIN_TF_DOCS -->
# AWS codepipeline example for Java based Container application module example
...
| <a name="module_codepipeline"></a> [codepipeline](#module\_codepipeline) | ../../modules/eks_java | n/a |
...
| <a name="input_artifact_path"></a> [artifact\_path](#input\_artifact\_path) | Enter the path to store artifact - S3 | `string` | `"codepipeline-new/"` | no |
...
| <a name="input_cidr_egress_rules"></a> [cidr\_egress\_rules](#input\_cidr\_egress\_rules) | Argocd deploy Security group egress rules | <pre>list(object({<br/>    from             = number<br/>    to               = number<br/>    protocol         = string<br/>    cidr_blocks      = list(string)<br/>    ipv6_cidr_blocks = list(string)<br/>    prefix_list_ids  = list(string)<br/>    description      = string<br/>  }))</pre> | n/a | yes |
...
| <a name="input_cidr_egress_rules_codebuild"></a> [cidr\_egress\_rules\_codebuild](#input\_cidr\_egress\_rules\_codebuild) | Codebuild security group egress rules | <pre>list(object({<br/>    from             = number<br/>    to               = number<br/>    protocol         = string<br/>    cidr_blocks      = list(string)<br/>    ipv6_cidr_blocks = list(string)<br/>    prefix_list_ids  = list(string)<br/>    description      = string<br/>  }))</pre> | n/a | yes |
| <a name="input_codebuild_role_service"></a> [codebuild\_role\_service](#input\_codebuild\_role\_service) | Aws service of the iam role | `list(string)` | <pre>[<br/>  "codebuild.amazonaws.com",<br/>  "eks.amazonaws.com"<br/>]</pre> | no |
...
| <a name="input_container_name"></a> [container\_name](#input\_container\_name) | Please enter application name to create images, Argocds and deploy to ECS or EKS | `string` | n/a | yes |
...
| <a name="input_publish_docker_registry"></a> [publish\_docker\_registry](#input\_publish\_docker\_registry) | Please enter ECR, JFROG or BOTH to publish docker images | `string` | n/a | yes |
...
| <a name="input_ssm_parameter_artifactory_helm_local_repo"></a> [ssm\_parameter\_artifactory\_helm\_local\_repo](#input\_ssm\_parameter\_artifactory\_helm\_local\_repo) | Enter the name value of jfrog artifactory Argocd local group name stored in ssm parameter | `string` | n/a | yes |
| <a name="input_ssm_parameter_artifactory_helm_virtual_repo"></a> [ssm\_parameter\_artifactory\_helm\_virtual\_repo](#input\_ssm\_parameter\_artifactory\_helm\_virtual\_repo) | Enter the name value of jfrog artifactory Argocd virtual group name storedin ssm parameter | `string` | n/a | yes |
