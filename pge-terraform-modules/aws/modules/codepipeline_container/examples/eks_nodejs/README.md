...
# Prerequisites : In the variables 'ssm\_parameter\_vpc\_id', 'ssm\_parameter\_subnet\_id1/2/3', 'ssm\_parameter\_artifactory\_host', 'ssm\_parameter\_artifactory\_nodejs\_repo', 'ssm\_parameter\_sonar\_host', 'ssm\_parameter\_environment', 'ssm\_parameter\_artifactory\_docker\_registry', 'secretsmanager\_github\_token\_secret\_name', 'secretsmanager\_artifactory\_token','secretsmanager\_artifactory\_user', 'secretsmanager\_sonar\_token', 'secretsmanager\_wiz\_client\_id', 'secretsmanager\_wiz\_client\_secret', 'argocd\_repo\_name', 'argocd\_repo\_path', 'secretsmanager\_codebuild\_github\_token\_secret\_name'.  Provide the suitable values respectively in tfvars.
...
| <a name="module_argocd_deployment"></a> [argocd\_deployment](#module\_argocd\_deployment) | app.terraform.io/pgetech/codebuild/aws | 0.1.29 |
...
| <a name="module_codepipeline"></a> [codepipeline](#module\_codepipeline) | ../../modules/eks_nodejs | n/a |
...
| <a name="module_security-group-codebuild"></a> [security-group-codebuild](#module\_security-group-codebuild) | app.terraform.io/pgetech/security-group/aws | 0.1.1 |
...
| [aws_ssm_parameter.artifactory_helm_local_repo](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/ssm_parameter) | data source |
| [aws_ssm_parameter.artifactory_helm_virtual_repo](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/ssm_parameter) | data source |
...
| <a name="input_app_owners"></a> [app\_owners](#input\_app\_owners) | Underscore separated application Development owners | `string` | n/a | yes |
| <a name="input_argocd_repo_name"></a> [argocd\_repo\_name](#input\_argocd\_repo\_name) | GitHub repository name that ArgoCD monitors for GitOps deployments (can be different from the application repo) | `string` | n/a | yes |
| <a name="input_argocd_repo_path"></a> [argocd\_repo\_path](#input\_argocd\_repo\_path) | Path within the GitOps repository where ArgoCD application manifests should be stored | `string` | `"argocd/applications"` | no |
...
| <a name="input_codestar_environment"></a> [codestar\_environment](#input\_codestar\_environment) | The environment is used for codestar notifications such as Dev, Test, QA, Prod. | `string` | n/a | yes |
| <a name="input_compute_type_argocd_deploy"></a> [compute\_type\_argocd\_deploy](#input\_compute\_type\_argocd\_deploy) | Information about the compute resources the build project will use in codebuild project | `string` | n/a | yes |
...
| <a name="input_concurrent_build_limit_argocd_deploy"></a> [concurrent\_build\_limit\_argocd\_deploy](#input\_concurrent\_build\_limit\_argocd\_deploy) | Maximum number of concurrent builds for the project in codebuild project | `number` | n/a | yes |
...
| <a name="input_container_name"></a> [container\_name](#input\_container\_name) | Enter the name of the container name. It should match the container name while creating the cluster. | `string` | n/a | yes |
...
| <a name="input_environment_image_argocd_deploy"></a> [environment\_image\_argocd\_deploy](#input\_environment\_image\_argocd\_deploy) | Docker image to use for codetest project. Valid values include Docker images provided by codebuild (e.g aws/codebuild/standard:2.0), Docker Hub images (e.g., hashicorp/terraform:latest), and full Docker repository URIs such as those for ECR (e.g., 137112412989.dkr.ecr.us-west-2.amazonaws.com/amazonlinux:latest). | `string` | n/a | yes |
...
| <a name="input_image_type"></a> [image\_type](#input\_image\_type) | Application image type | `string` | n/a | yes |
...
| <a name="input_node_build"></a> [node\_build](#input\_node\_build) | Enter the build or install command to build nodejs webapp application | `string` | n/a | yes |
| <a name="input_node_runtime"></a> [node\_runtime](#input\_node\_runtime) | Enter the runtime version of node - variable in buildspec yml | `string` | `null` | no |
...
| <a name="input_s3_block_public_policy"></a> [s3\_block\_public\_policy](#input\_s3\_block\_public\_policy) | Bucket name for codepipeline | `bool` | `true` | no |
| <a name="input_s3_force_destroy"></a> [s3\_force\_destroy](#input\_s3\_force\_destroy) | Bucket name for codepipeline | `bool` | `true` | no |
| <a name="input_secretsmanager_artifactory_secret_name"></a> [secretsmanager\_artifactory\_secret\_name](#input\_secretsmanager\_artifactory\_secret\_name) | Secrets Manager path for Artifactory credentials | `string` | n/a | yes |
...
| <a name="input_secretsmanager_codebuild_github_token_secret_name"></a> [secretsmanager\_codebuild\_github\_token\_secret\_name](#input\_secretsmanager\_codebuild\_github\_token\_secret\_name) | Secrets Manager path for GitHub PAT with read/write access to the GitOps repository | `string` | n/a | yes |
...
| <a name="input_source_location_argocd_deploy"></a> [source\_location\_argocd\_deploy](#input\_source\_location\_argocd\_deploy) | Location of the source code from git or s3 to build codescan project. | `string` | n/a | yes |
...
| <a name="input_ssm_parameter_artifactory_helm_local_repo"></a> [ssm\_parameter\_artifactory\_helm\_local\_repo](#input\_ssm\_parameter\_artifactory\_helm\_local\_repo) | Enter the name value of jfrog artifactory helm chart local group name stored in ssm parameter | `string` | n/a | yes |
| <a name="input_ssm_parameter_artifactory_helm_virtual_repo"></a> [ssm\_parameter\_artifactory\_helm\_virtual\_repo](#input\_ssm\_parameter\_artifactory\_helm\_virtual\_repo) | Enter the name value of jfrog artifactory helm chart virtual group name storedin ssm parameter | `string` | n/a | yes |
...
| <a name="input_ssm_parameter_artifactory_repo_name"></a> [ssm\_parameter\_artifactory\_repo\_name](#input\_ssm\_parameter\_artifactory\_repo\_name) | Enter the name value of jfrog artifactory stored in ssm parameter | `string` | n/a | yes |
...
| <a name="input_unit_test_commands"></a> [unit\_test\_commands](#input\_unit\_test\_commands) | Enter the unit test commands for nodejs webapp | `string` | n/a | yes |
