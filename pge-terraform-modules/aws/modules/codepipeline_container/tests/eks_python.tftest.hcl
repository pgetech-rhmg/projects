...
variables {
  ...
  project_root_directory                      = "reference-pipelines/python/ecs-eks/pge-container-python-example"
  python_runtime                              = "3.9"
  unit_test_commands                          = "pip install pytest && pip install pytest-cov && py.test --cov-report xml:coverage.xml --cov=. --junitxml=result.xml app/app_test.py"
  image_type                                  = "pge-app"
