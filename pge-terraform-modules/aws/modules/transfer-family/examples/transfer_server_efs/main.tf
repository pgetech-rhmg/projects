...
*/
#
# Filename    : modules/glue/examples/transfer_server_efs/main.tf
# Date        : 20 September 2022
# Author      : TCS
# Description : The Terraform usage example creates transfer server for efs.
...
module "transfer_access" {
  ...
  home_directory = "/${module.efs.efs_id}/"
  posix_profile = {
    gid            = var.gid
    uid            = var.uid
...
module "efs" {
  ...
  kms_key_id      = null # replace with module.kms_key.key_arn, after key creation if the DataClassification is not Internal/Public
