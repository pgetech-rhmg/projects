...
resource "aws_codepipeline_webhook" "codepipeline_webhook" {
  ...
  # AWS API does not return secret_token (write-only for security), causing Terraform to detect drift on every plan.
  # Ignoring authentication_configuration prevents unnecessary webhook recreation. (AWS Support Case: 176973092200047)
  # To rotate the secret, run: terraform taint aws_codepipeline_webhook.codepipeline_webhook
  lifecycle { ignore_changes = [authentication_configuration] }
}
