...
### CodePipeline Webhook Secret Token Drift

**Issue:** Terraform detects drift in the `authentication_configuration.secret_token` field on every `terraform plan`/`apply`, causing the webhook to be replaced unnecessarily.

**Root Cause:** The AWS CodePipeline API intentionally does not return the `secret_token` value when describing a webhook. This is an AWS security measure—the secret is write-only. During `terraform plan`, Terraform queries the AWS API to compare current infrastructure against state. Since AWS returns an empty/null value for `secret_token`, Terraform interprets this as drift and forces resource replacement on every apply.

**Resolution:** This module includes a `lifecycle` block with `ignore_changes` for `authentication_configuration` to prevent unnecessary webhook recreation:
...
**Secret Rotation:** If you need to rotate the webhook secret, you have the following options:
- Manually run `terraform taint aws_codepipeline_webhook.codepipeline_webhook` to force recreation
- Modify the `keepers` value in the `random_password` resource (e.g., change `repo_name`)

**Reference:** Confirmed by AWS Premium Support (CodePipeline Team) - This is expected and intentional security behavior. The API does not expose stored secrets. (AWS Support Case ID: 176973092200047)
