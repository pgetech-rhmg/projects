...
variable "queues" {
  ...
  default     = null
  validation {
    condition     = can(regex("^[a-zA-Z0-9]", var.queues))
    error_message = "Error! enter the valid Amazon MQ broker destination queue to consume."
  }
}
...
variable "source_access_configuration" {
  ...
  validation {
    ...
    error_message = "Error! The valid values for type are VPC_SUBNET, VPC_SECURITY_GROUP,  VIRTUAL_HOST, and BASIC_AUTH.  For type VPC_SUBNET the value should be subnet : subnet_id and for type VPC_SECURITY_GROUP the value should be security_group : security_group_id."
  }
}