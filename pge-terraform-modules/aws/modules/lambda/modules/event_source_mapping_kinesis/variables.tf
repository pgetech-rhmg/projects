...
variable "starting_position" {
  ...
  validation {
    condition = anytrue([
      var.starting_position == "AT_TIMESTAMP",
...
variable "function_response_types" {
  description = " list of current response type enums applied to the event source mapping for AWS Lambda checkpointing.Valid value: ReportBatchItemFailures"
  type        = list(string)
  default     = []
  validation {
    condition = alltrue([
