...
variable "source_access_configuration" {
  ...
  validation {
    ...
    error_message = "Error! Enter the valid values for source_access_configuration."
  }
}

variable "self_managed_event_source_endpoints" {
  description = "A map of endpoints for the self managed source.The key should be KAFKA_BOOTSTRAP_SERVERS and the value should be a string with a comma separated list of broker endpoints."
  type        = map(string)
  validation {
    condition     = contains(keys(var.self_managed_event_source_endpoints), "KAFKA_BOOTSTRAP_SERVERS")
    error_message = "Error! valid key should be KAFKA_BOOTSTRAP_SERVERS."
  }
  validation {
    condition     = alltrue([for ki, vi in var.self_managed_event_source_endpoints : can(regex("^[a-zA-Z0-9]+.[a-zA-Z0-9]+.[a-zA-Z]+:[0-9]+", vi))])
    error_message = "Error! the value should be a string with a comma separated list of broker endpoints."
  }
}
