...
*/
#
#  Filename    : aws/modules/codepipeline/examples/codepipeline_ecs_java_bluegreen/main.tf
...
module "codepipeline" {
  source  = "app.terraform.io/pgetech/codepipeline/aws//modules/codepipeline_ecs_java"
  version = "0.0.80"
  ...
  artifactory_maven_repo      = data.aws_ssm_parameter.artifactory_maven_repo.value
...
module "security-group-codebuild" {
  ...
  name              = "sg_codebuild_codetest_${var.codepipeline_name}"
...
module "codetest_iam_role" {
  ...
  name        = "${var.codepipeline_name}-iam-policy"
