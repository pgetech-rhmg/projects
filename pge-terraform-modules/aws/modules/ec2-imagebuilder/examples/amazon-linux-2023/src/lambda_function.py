...
def email_notification_prod(ami_id):
    ...
    SUBJECT = "New PGE Amazon " + " Golden AMI " + ami_id + " is released for NonProd"   #update
    CHARSET = "UTF-8"
    BODY_TEXT = "Test email"
    BODY_HTML = """<html>
        <body>
        <title></title>
        <p>Cloud COE have released a newer version of PGE Amazon.  Golden AMI to all PGE NonProd AWS accounts. Newly released AMI Id is accessible via AWS Parameter Store</p>
        <ul>
            <li> AMI ID : <strong><big> {ami_id:} </big></strong></li>
        </ul>
        <p> Please use this latest AMI to launch EC2 in your Non-Prod AWS accounts and validate.</p>
