...
*/
#
# Filename    : modules/ecs/examples/ecs_ec2_fluentbit/main.tf
# Date        : 29 Nov , 2022
# Author      : TekYantra
...
module "ecs_task_definition" {
  ...
  container_definition     = jsonencode([module.ecs_container_definition.json_map_object, module.fluentbit_container.json_map_object])
  tags                     = merge(module.tags.tags, local.optional_tags)
}
...
module "fluentbit_container" {
  ...
  essential                    = var.fluentbit_essential
  readonly_root_filesystem     = var.readonly_root_filesystem
  environment                  = var.container_environment
  privileged                   = var.privileged
  extra_hosts                  = var.extra_hosts
  ...
  pseudo_terminal              = var.pseudo_terminal
  interactive                  = var.interactive
  port_mappings                = var.fluentbit_port_mappings
