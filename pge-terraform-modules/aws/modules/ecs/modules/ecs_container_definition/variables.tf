...
variable "wiz_log_configuration" {
  type        = any
  description = "Log configuration options to send to a custom log driver for the container. For more details, see https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_LogConfiguration.html"
  default     = null
}
...
variable "create_wiz_container" {
  description = "Determines whether to create the wiz container definition"
  type        = bool
  default     = true
}
...
variable "wiz_firelens_configuration" {
  type = object({
    type    = string
    options = map(string)
  })
