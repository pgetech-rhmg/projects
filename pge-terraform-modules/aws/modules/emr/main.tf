...
# Module      : EMR cluster
# Description : This terraform module creates an EMR cluster
...
dynamic "core_instance_fleet" {
        ...
        content {
          ...
          dynamic "ebs_config" {
            ...
            content {
              ...
              size                 = try(ebs_config.value.size, 256)
    ...
    dynamic "core_instance_group" {
        ...
        instance_count = try(core_instance_group.value.instance_count, 1)
  ...
  dynamic "ec2_attributes" {
    ...
    dynamic "master_instance_fleet" {
      for_each = var.master_instance_fleet != null ? { "master" = var.master_instance_fleet } : {}
...
resource "aws_emr_instance_fleet" "this" {
  for_each = var.task_instance_fleet != null ? { "task" = var.task_instance_fleet } : {}
...
resource "aws_emr_instance_group" "this" {
  for_each = var.task_instance_group != null ? { "task" = var.task_instance_group } : {}
...
resource "aws_emr_managed_scaling_policy" "this" {
  count = var.managed_scaling_policy != null ? 1 : 0
...
##### EMR Autoscaling IAM Role #####

locals {
  # Autoscaling not supported when using instance fleets
  create_autoscaling_iam_role = var.create_autoscaling_iam_role && length(merge(var.core_instance_fleet, var.master_instance_fleet)) == 0
...
resource "aws_iam_role" "autoscaling" {
  ...
  name        = local.autoscaling_iam_role_name
  ...
  tags = merge(local.module_tags, var.iam_role_tags)
}
...
data "aws_iam_policy_document" "autoscaling" {
  ...
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws:elasticmapreduce:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:*"]
    }
  }
}
...
locals {
  ...
  merged_service_iam_role_policies = merge({ AmazonEMRServicePolicy_v2 = "arn:aws:iam::aws:policy/service-role/AmazonEMRServicePolicy_v2" }, var.service_iam_role_policies)
}

resource "aws_iam_role" "service" {
  count = local.create_service_iam_role ? 1 : 0

  name        = local.service_iam_role_name
  path        = var.iam_role_path
...
resource "aws_iam_role_policy_attachment" "service" {
  for_each = { for k, v in local.merged_service_iam_role_policies : k => v if local.create_service_iam_role }
...
resource "aws_iam_policy" "service_pass_role" {
  ...
  name        = local.service_pass_role_policy_name
  path        = var.iam_role_path
...
resource "aws_security_group_rule" "master" {
  ...
  source_security_group_id = try(each.value.source_slave_security_group, false) ? aws_security_group.slave[0].id : try(each.value.source_service_security_group, false) ? aws_security_group.service[0].id : lookup(each.value, "source_security_group_id", null)
}
...
resource "aws_security_group_rule" "slave" {
  ...
  prefix_list_ids          = lookup(each.value, "prefix_list_ids", null)
  self                     = lookup(each.value, "self", null)
  source_security_group_id = try(each.value.source_master_security_group, false) ? aws_security_group.master[0].id : try(each.value.source_service_security_group, false) ? aws_security_group.service[0].id : lookup(each.value, "source_security_group_id", null)
}
